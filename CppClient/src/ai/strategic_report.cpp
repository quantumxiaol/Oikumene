...
StrategicReport BuildStrategicReport(const Simulation& sim, PolityId polity_id, const StrategicReportOptions& options) {
    const auto* polity = PolityById(sim.Polities(), polity_id);
    ...
    result.report = StrategicSummaryToJson(sim, *polity, options);
    result.candidate_actions = BuildCandidateActions(sim, *polity, options);
    return result;
}

...
void SettlementSystem::UpdateSettlements(World& world, const SimulationParams& params, Turn turn,
    ...
    for (auto& settlement : settlements) {
        ...
        const int build_interval = settlement.level == SettlementLevel::Village
                                       ? params.village_improvement_build_interval
        ...
        if (should_build_now && existing_improvements < improvement_limit) {
            BuildBestImprovement(world, settlement, DesiredImprovement(world, settlement, params, effects), turn,
                                 event_log);
        }
        ...
        settlement.carrying_capacity_ratio =
            settlement.carrying_capacity <= 0.0F
                ? 0.0F
        ...
        if (settlement.stockpile.food > static_cast<float>(settlement.population) * 0.25F &&
            ...
            const int loss = std::max(1, static_cast<int>(static_cast<float>(settlement.population) * 0.025F *
                                                          effects.famine_severity_multiplier));
